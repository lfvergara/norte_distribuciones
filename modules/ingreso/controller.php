...
class IngresoController {
	...
	function listar($arg) {
    	...
    	$select = "i.ingreso_id AS INGRESO_ID, i.fecha AS FECHA, prv.razon_social AS PROV, ci.denominacion AS CONDI,CONCAT(tf.nomenclatura, ' ', LPAD(i.punto_venta, 4, 0), '-', LPAD(i.numero_factura, 8, 0)) AS FACTURA, i.costo_total AS TOTAL, i.costo_distribucion AS COSTDIST, i.costo_total_iva AS TIVA, cp.denominacion AS CP, CASE WHEN (SELECT COUNT(ccp.ingreso_id) FROM cuentacorrienteproveedor ccp WHERE ccp.ingreso_id = i.ingreso_id) > 1 THEN 'none' ELSE 'inline-block'END AS DSP_BTN_EDIT ";
