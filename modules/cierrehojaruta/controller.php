...
class CierreHojaRutaController {
	...
	function buscar() {
    	...
    	$select = "chr.cierrehojaruta_id AS CHRID, CONCAT(date_format(chr.fecha, '%d/%m/%Y'), ' ', chr.hora) AS FECHA, ROUND(chr.rendicion, 2) AS RENDICION, ROUND(chr.facturacion, 2) AS FACTURACION, chr.hojaruta_id AS HOJARUTA, c.denominacion AS FLETE";
    	$from = "cierrehojaruta chr INNER JOIN cobrador c ON chr.cobrador = c.cobrador_id";
