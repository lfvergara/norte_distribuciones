...
class FacturaPDFProcesoLote extends View {

    public function facturaAPL($egresodetalle_collection, $obj_configuracion, $obj_egreso) {
        ...
        $telefono_vendedor = '';
        $infocontacto_vendedor = $obj_vendedor->infocontacto_collection;
        foreach ($infocontacto_vendedor as $c=>$v) {
            if($v->denominacion == 'Celular') $telefono_vendedor = 'Cel: ' . $v->valor;
        }

        $telefono_flete = '';
        $infocontacto_flete = $obj_cliente->flete->infocontacto_collection;
        foreach ($infocontacto_flete as $c=>$v) {
            if($v->denominacion == 'Celular') $telefono_flete = 'Cel: ' . $v->valor;
        }

        $vendedor = $obj_vendedor->apellido . ' ' . $obj_vendedor->nombre . ' (' . $telefono_vendedor .')';
        $flete = $obj_cliente->flete->denominacion  . ' (' . $telefono_flete .')';
    ...
    public function remitoR($egresodetalle_collection, $obj_configuracion, $obj_egreso) {
        $gui_html = file_get_contents("static/common/plantillas_facturas/plantilla_html.html");
